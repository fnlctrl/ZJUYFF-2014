...
?>
    <table id="obj-v<?php echo $value->id;?>">
        ...
        <tr class="link-img1"><td>参赛作品</td><td><a href="<?php echo wrapImage(1, $value->id, $value->pictype1 ? '.jpg' : $value->suffix1);?>" target="_blank">点击查看参赛作品</a> <?php if (isunknown($value->pictype1)) echo "未知格式的图片";?></td></tr>
        <tr class="link-img2"><td>原版海报</td><td><a href="<?php echo wrapImage(2, $value->id, $value->pictype2 ? '.jpg' : $value->suffix2);?>" target="_blank">点击查看原版海报</a> <?php if (isunknown($value->pictype2)) echo "未知格式的图片";?></td></tr>
<?php
    foreach ($value->teammate as $vvalue) {
?>
